...
abstract: | 
  The current study examined potential sources of variation in the frequency of visiting social media sites and the overall time spent on social media each day. In particular, the relations between the Dark Triad traits (i.e., Machiavellianism, psychopathy, and narcissism) and social media use are discussed. Furthermore, interactions between the Dark Triad and gender are explored, suggesting that these two factors interact in predicting social media use, depending on the social media site in question.  Overall, there are robust and important personality-related individual differences in social media use, and it is suggested that future research is needed to better clarify why certain social media platforms attract those with "darker" personalities.
...
## Yufei: thanks for having all these installation codes for reviewers!!
...
## Yufei: (strength 1) the detailed comments really help me a lot in understanding the codes!
```
...
## Yufei: (strength 2) I really like the way you separate the codes and highlight the title within comments
## It's super clear to under the structure of the codes!
...
zo %>%
  apa_table(caption = "Zero-order correlations with frequency of social media use, length spent on social media per day, and the Dark Triad traits.",
            note = paste0("* \\textit{p} < .05, ** \\textit{p} < .01, *** \\textit{p} < .001. Values above the diagonal are for women (\\textit{n} = ",
                          ...
                          median(zo_mal$n),
                          "). P-values were adjusted using the Holm-Bonferonni method and were adjusted separately for the male and female sub-samples. Frequency of social media use and length spent on social media were collapsed across all sites."),
            align = "l",
            escape = FALSE)
...
In terms of random effects, we allowed intercepts to vary across subjects. It would have also been defensible to run the model as a two-level cross-classified model, nesting observations within both subjects and social media sites. However, we were interested in the effects of specific social media sites and thus included them as fixed effects in our model. In the present case, grouping by subject explained `r spround(freq_icc * 100)`% of the variation in frequency of social media use and `r spround(len_icc * 100)`% of the variation in the length of social media use before addition of any fixed-effect predictors. We added fixed effects to our model in the following order: (1) gender, (2) the Dark Triad traits, (3) the specific social media site, (4) the interaction between gender and the Dark Triad traits, (5) the interaction between gender and the social media sites, (6) the interaction between the Dark Triad traits and the  social media sites, and, finally, (7) the three-way interaction between the Dark Triad traits, social media sites, and gender. Results of the model comparisons can be see in Table `r (table_count <- table_count + 1)`. Since the three-way interaction significantly improved model fit for the length of social media use model, we retained all predictors. Although the three-way model for the frequency of visiting social media only marginally improved model fit, we retained all predictors to allow for easy comparison across both series of models.
...
Degrees of freedom were estimated using Satterthwaite's method in *lmerTest* [@R-lmerTest]. The reference groups for both models is Facebook. Results revealed that compared to men, women tended to visit Facebook more often, *b* = 0.45, *SE* = .19, *t*(791.03) = 2.30, *p* = .022. However, men appeared to visit Reddit [*b* = 1.23, *SE* = .23, *t*(1571.11) = 5.31, *p* < .001] and YouTube [*b* = 1.07, *SE* = .23, *t*(1571.11) = 4.62, *p* < .001] more than women. In regard to the Dark Triad traits, those higher in narcissism reported visiting Instagram [*b* = .79, *SE* = .31, *t*(1571.11) = 2.56, *p* = .011] and Snapchat [albeit marginally; *b* = .56, *SE* = .31, *t*(1571.11) = 1.81, *p* = .07] more frequently than Facebook. Interestingly, it appears the effect of narcissism on Instagram use interacted with gender, such that female narcissists visited Instagram less than male narcissists, *b* = -0.79, *SE* = .35, *t*(1571.11) = -2.27, *p* = .023. Those high in Machiavellianism, on the other hand, visited Reddit more than Facebook, *b* = .85, *SE* = .30, *t*(1571.11) = 2.84, *p* = .004. This effect again depended on gender. That is, women high in Machiavellianism  visited Reddit less than men high in Machiavellianism, *b* = -0.92, *SE* = .38, *t*(1571.11) = -2.39, *p* = .017. Finally, the only effect to arise concerning psychopathy was an interaction between psychopathy, gender, and social media site. Specifically, women scoring higher in psychopathy reported visiting Reddit more than Facebook, *b* = .84, *SE* = .42, *t*(1571.11) = 1.99, *p* = .047.

## Length of Social Media Use (in minutes)

While women tended to visit social media sites more overall, analyzing the total time spent on social media per day (in minutes) revealed women and men did not significantly differ from one another in overall time spent on social media (*p* > .29). Though, results similarly indicated that males appeared to report spending more minutes per day on Reddit [*b* = 25.63, *SE* = 8.09, *t*(1552.29) = 3.17, *p* = .002] and YouTube [*b* = 37.96, *SE* = 8.09, *t*(1552.29) = 4.69, *p* < .001]. Examining the Dark Triad traits revealed that when compared to time spent on Facebook, those higher in narcissism reported spending less minutes per day on Reddit [*b* = -23.23, *SE* = 10.70, *t*(1551.95) = -2.17, *p* = .030]. The influence of narcissism on time spent on Reddit each day depended on one's gender. Female narcissists spent more time on Reddit than male narcissists, *b* = 22.09, *SE* = 12.14, *t*(1552.04) = 1.82, *p* = .07. This effect only approached conventional levels of significance, meaning the result should be interpreted with caution.

Conversely, those high in Machiavellianism spent significantly less minutes per day on YouTube  [*b* = -25.28, *SE* = 10.38, *t*(1551.95) = -2.44, *p* = .015]. Again, this effect changed as a function of gender, such that women high in Machiavellianism actually spent more time per day on YouTube [*b* = 24.44, *SE* = 13.35, *t*(1552.71) = 1.83, *p* = .07] and Twitter [*b* = 25.33, *SE* = 13.36, *t*(1552.99) = 1.90, *p* = .06] than men high in  Machiavellianism. We should further note that these interactions with gender were only approaching conventional levels of significance. Lastly, results suggested those high in psychopathy spent more minutes per day on YouTube than Facebook [*b* = 36.53, *SE* = 12.23, *t*(1551.95) = 2.99, *p* = .003], and this finding also appeared to depend on gender. Female psychopaths spent less time on YouTube than male psychopaths, *b* = -34.66, *SE* = 14.77, *t*(1552.99) = -2.35, *p* = .019.

Overall, the results above suggest the Dark Triad and gender are important factors in predicting social media use, depending on the social media site in question (i.e., Facebook, Instagram, Reddit, Snapchat, Twitter, and YouTube). While women tended to visit social media sites more overall, women and men did not differ from one another in overall time spent on social media. Though, men did report visiting YouTube and Reddit more than women, and men also spent more time each day on these sites. Examining the Dark Triad traits revealed that those higher in narcissism reported visiting Instagram more frequently than Facebook. Though, narcissistic individuals didn't actually spend significantly more time on the site. Rather, narcissistic individuals spent less minutes per day on Reddit and YouTube. Those high in Machiavellianism, on the other hand, visited Reddit more than Facebook and spent less time per day on YouTube (relative to time spent on Facebook). Finally, those scoring high in psychopathy spent more time per day on YouTube than Facebook. While many of these effects interacted with gender, it appears that there are robust and important personality-related individual differences in social media use. Future research is needed to better clarify why certain social media platforms (e.g., YouTube) attract those with "darker" personalities (e.g., Psychopaths) and why some of these effects depend on one's gender.
...
## Yufei: (one thing I learned) The whole apa format paper written with R is so impressive! By reading your codes I got a chance to see how this done by R. The coding style is so tidy and consistent and I will try to keep my codes in the same clean way. All in all, this work is an awesome sample for people to learn about tidying data/ploting/using papaja with detailed comments and explanation!

## Yufei: (improvement) The codes are tidy and compact; the plots looks pretty and clear. If I really have to find something that needs improvement, I would ask if you could add a little brief description to your custom functions? I really want to adopt your awesome functions!
