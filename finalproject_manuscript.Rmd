...
corr_data <- ash_data %>% 
  ...
  select(id, gender, mean_freq, sum_freq, sum_len, self, society, 
...
model1 <- lmer(len ~ gender*sns + (1 | id), data = genderxsns_data)
summary(model1)

###########################################################

#same plot as above...
#But, instead of minutese on social media, the DV here was a likert scale
...
mach <- lmer(len ~ dd_mach_comp + (1 | id), data = ash_data)

mach_gender <- lmer(len ~ dd_mach_comp*gender + (1 | id),
                       data = ash_data)

anova(mach, mach_gender) #adding gender doesn't do anything

mach_sns <- lmer(len ~ dd_mach_comp*sns + (1 | id),
                       data = ash_data)
anova(mach, mach_sns) #mach_sns model is best thus far
summary(mach_sns) 
#people high in mach spend more time each day on insta and snapchat

# Machiavellianism x gender x sns model
mach_sns_gender <- lmer(len ~ dd_mach_comp*sns*gender + (1 | id),
                        data = genderxsns_data)

summary(mach_sns_gender) #nothing shakes out with 3 way interaction!!

# BEST MACH MODEL IS MACH_SNS (no gender)
...
narc <- lmer(len ~ dd_narc_comp + (1 | id), data = ash_data)

narc_gender <- lmer(len ~ dd_narc_comp*gender + (1 | id),
                       data = ash_data)

anova(narc, narc_gender) #adding gender interaction doesn't do anything

narc_sns <- lmer(len ~ dd_narc_comp*sns + (1 | id),
                       data = ash_data)
anova(narc, narc_sns) #narc_sns model is best thus far
summary(narc_sns) 

#people high in Narcissism spend more time each day on snapchat
...
narc_sns_gender <- lmer(len ~ dd_narc_comp*sns*gender + (1 | id),
                        data = genderxsns_data)

narc_sns <- lmer(len ~ dd_narc_comp*sns + (1 | id),
                       data = genderxsns_data) #need to re-run model with same data

anova(narc_sns, narc_sns_gender) #model with 3 way interaction fits the best!!

# BEST MACH MODEL IS MACH_SNS_GENDER 
summary(narc_sns_gender)
...
psyc <- lmer(len ~ dd_psyc_comp + (1 | id), data = ash_data)

psyc_gender <- lmer(len ~ dd_psyc_comp*gender + (1 | id),
                       data = ash_data)

anova(psyc, psyc_gender) #adding gender interaction doesn't do anything

psyc_sns1 <- lmer(len ~ dd_psyc_comp*sns + (1 | id),
                       data = ash_data)
anova(psyc, psyc_sns1) #psyc_sns model is best thus far
summary(psyc_sns1) #people high in psychopathy spend more time each day on youtube and snapchat
...
### hmm... doesn't really look like much is there

# Psychopathy x gender x sns model
psyc_sns_gender <- lmer(len ~ dd_psyc_comp*sns*gender + (1 | id),
                        data = genderxsns_data)
summary(psyc_sns_gender) #according to this model, differences only arise with reddit
