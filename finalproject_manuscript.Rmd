...
create_group <- function(data, group_var) {
    group_var <- case_when(
    data > quantile(data, .75, na.rm = TRUE) ~ "High",
    data < quantile(data, .25, na.rm = TRUE) ~ "Low",
    TRUE ~ "Mid")
    group <- tibble(group_var)
    data$group <- group
}

create_group(genderxsns_data$dd_mach_comp, test_mach_group) #not working... keeps ccohercing into list
...
psyc_sns <- lmer(len ~ dd_psyc_comp*sns + (1 | id),
             data = ash_data)
summary(psyc_sns)
