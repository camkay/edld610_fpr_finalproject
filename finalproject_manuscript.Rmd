...
# create a function for calculating the mean of time responses given as a range
range_mean <- function(x) {
  as.character(mean(as.numeric(strsplit(str_extract(x, "\\d*-\\d*"), "-")[[1]])))
}

# replace all ranges with the mean of the ranges
...
data1 %<>%
  mutate(# create a new column for the unit the response was given in
         len_unit  = case_when(grepl("(hour|hr)", len) ~ 60,
                               TRUE                    ~ 1),
         # replace characgter numbers with numeric numbers
         ...
         # multiple the length value by the length unit to give the total number in minutes
         len       = len_clean * len_unit) %>%
  # drop the len_unit and len_clean columns
  select(-len_unit, -len_clean)

# factorize and label factors
data1 %<>%
  mutate(# factorize gender
