...
With respect to concrete hypotheses, we expected that (1) narcissism would positively correlate with both the frequency of using social media and the amount of time spent on social media, independent of the other Dark Triad traits and irrespective of the exact social media site in question. We also expected (2) psychopathy would positively correlate with the frequency of general social media use and time spent on social media sites, even when controlling for the effect of narcissism and Machiavellianism and irrespective of the exact social media site in question. Finally, we expected (3) Machiavellianism would be independent of the other Dark Triad traits—negatively correlated with both frequency of social media use and time spent on social media sites, once again irrespective of the exact social media site in question. Differences in whether dark personalities use certain sites more than other sites and interactions with gender will be investigated in a purely exploratory fashion. 
...
zo %>%
  apa_table(caption = "Zero-order correlations for frequency and length of social media use and the Dark Triad traits.",
            note = paste0("* \\textit{p} < .05, ** \\textit{p} < .01, *** \\textit{p} < .001. Values above the diagonal are for women (\\textit{n} = ",
                          ...
                          median(zo_mal$n),
                          "). P-values were adjusted using the Holm-Bonferonni method and were adjusted separately for the male and female sub-samples."),
            align = "l",
            escape = FALSE)
...
Zero-order correlations among all variables are listed in Table `r (table_count <- table_count + 1)`, which seems to suggest that at conventional significant levels there is no relationship between the Dark Triad and either the frequency of social media use or the average length of social media use per day. However, the frequency and length measures included in the matrix are collapsing across social media sites. In order to leverage the multiple observations gathered for every participant, we opted to run two series of linear mixed-effects model. In terms of predictors, both models were identical. The models only differed by outcome variable:  The first model had frequency of social media use as an outcome variable. The second model had length of social media use per day (in minutes) as the outcome variable.
...
In terms of random effects, we allowed random intercepts across subjects. It would have been defensible to run the model as a two-level cross-classified model, nesting observations within both subjects and social media sites. However, we were interested in the effects of social media site and thus included them as fixed effects in our model. In the present case, grouping by subject explained `r spround(freq_icc * 100)`% of the variation in frequency of social media use and `r spround(len_icc * 100)`% of the variation in the length of social media use. We added fixed effects to our model in the following order: (1) gender, (2) the Dark Triad traits, (3) the specific social media site, (4) the interaction between gender and the Dark Triad traits, (5) the interaction between gender and the social media sites, (6) the interaction between the Dark Triad traits and the  social media sites, and, finally, (7) the three-way interaction between the Dark Triad traits, social media sites, and gender. Results of the model comparisons can be see in Table `r (table_count <- table_count + 1)`. Note the final model above included all of the previously mentioned fixed effects, except for the gender x Dark Triad interaction. Model comparisons revealed that the inclusion of this interaction term did not significantly improve model fit. The full model for the two models was:
...
**NEED TO UPDATE WITH THREE-WAY INTERACTION.** 
...
Results revealed that compared to males, females tended to use social media sites more frequently, overall, *b* = 0.45, *SE* = .19, *t*(791.03) = 2.30, *p* = .022. However, males appeared to use Reddit [*b* = 1.23, *SE* = .23, *t*(1571.11) = 5.31, *p* < .001] and Youtube [*b* = 1.07, *SE* = .23, *t*(1571.11) = 4.62, *p* < .001] more frequently than females. 

In regard to the Dark Triad traits, those higher in narcissism reported using Instagram [*b* = .79, *SE* = .31, *t*(1571.11) = 2.56, *p* = .011] and--marginally--Snapchat [*b* = .56, *SE* = .31, *t*(1571.11) = 1.81, *p* = .07] more frequently than Facebook. Interestingly, it appears this effect of Narcissism on Instagram use interacted with gender, such that female narcissists used Instagram less than male narcissists, *b* = -0.79, *SE* = .35, *t*(1571.11) = -2.27, *p* = .023. Those high in Machiavellianism, on the other hand, used Reddit more than Facebook, *b* = .85, *SE* = .30, *t*(1571.11) = 2.84, *p* = .004. This effect again depended on gender. That is, females high in Machiavellianism  used Reddit less than males high in Machiavellianism, *b* = -0.92, *SE* = .38, *t*(1571.11) = -2.39, *p* = .017. Finally, the only effect to arise concerning psychopathy was an interaction between psychopathy, gender, and social media site. Specifically, females scoring higher in psychopathy reported using Reddit more than Facebook, *b* = .84, *SE* = .42, *t*(1571.11) = 1.99, *p* = .047.

*Length of Social Media Use (in minutes)*

Contrary to the model above, analyzing the total reported minutes on social media per day revealed males and females did not significantly differ from one another in overall social media use (*p* > .29). However, results were consistent in that males appeared to report spending more minutes per day on Reddit [*b* = 25.63, *SE* = 8.09, *t*(11552.29) = 3.17, *p* = .002] and Youtube [*b* = 37.96, *SE* = 8.09, *t*(11552.29) = 4.69, *p* < .001].

Examining the Dark Triad traits revealed that when compared to time spent on Facebook, those higher in narcissism reported spending less minutes per day on Reddit [*b* = -23.23, *SE* = 10.70, *t*(1551.95) = -2.17, *p* = .030] and--marginally--Youtube, *b* = -19.29, *SE* = 10.70, *t*(1551.95) = -1.80, *p* = .07. The influence of narcissism on time spent on Reddit each day marginally depended on one's gender. Female narcissists spent more time on Reddit than male narcissists, *b* = 22.09, *SE* = 12.14, *t*(1552.04) = 1.82, *p* = .07. Conversely, those high in Machiavellianism spent significantly less minutes per day on Youtube  [*b* = -25.28, *SE* = 10.38, *t*(1551.95) = -2.44, *p* = .015] and marginially less minutes per day on Twitter, *b* = -18.79, *SE* = 10.38, *t*(1551.95) = -1.81, *p* = .07. Though, both of these effects marginally changed as a function of gender, such that females high in Machiavellianism actually spent more time per day on Youtube [*b* = 24.44, *SE* = 13.35, *t*(1552.71) = 1.83, *p* = .07] and Twitter [*b* = 25.33, *SE* = 13.36, *t*(1552.99) = 1.90, *p* = .06] than males high in  Machiavellianism. Lastly, results suggested those high in psychopathy spent more minutes per day on Youtube than Facebook, *b* = 36.53, *SE* = 12.23, *t*(1551.95) = 2.99, *p* = .003. This finding also appeared to depend on gender, such that female psychopaths spent less time on Youtube than male psychopaths, *b* = -34.66, *SE* = 14.77, *t*(1552.99) = -2.35, *p* = .019.
...
# round tables to appropriate number of digits
...
model_comps_comb <- apply(model_comps_comb, 
                          MARGIN = c(1, 2), 
                          function(string) gsub(".000", "<.000", string))
