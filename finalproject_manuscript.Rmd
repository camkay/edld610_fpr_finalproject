...
zo %>%
  apa_table(caption = "Zero-order correlations for frequency and length of social media use and the Dark Triad traits.",
            note = paste0("* \\textit{p} < .05, ** \\textit{p} < .01. Values above the diagononal are for women (\\textit{n} = ",
                         ...
                         median(zo_mal$n),
                         "). P-values were adjusted using the Holm-Bonferonni method (Holm, 1978) and were adjusted seperately for the male and female subsamples."),
            align = "l",
            escape = FALSE)
```

See Table `r (table_count <- table_count + 1)` for zero-order correlations. 

```{r data1_model_comps_freq, include = FALSE}
# model 0 unconditional
model_freq_0 <- lmer(freq ~ (1 | id) + (1 | sns), data = data1)

# model 1 gender
model_freq_1 <- lmer(freq ~ gender + 
                            (1 | id) + (1 | sns), data = data1)
...
# model 3 dark triad
model_freq3 <- lmer(freq ~ dd_mach_comp + 
                          dd_narc_comp + 
                          dd_psyc_comp + 
                          sns +
                          dd_mach_comp * sns + 
...
summary(model_freq_3)

model_len_3 <- lmer(len ~ dd_mach_comp + 
                          dd_narc_comp + 
                          dd_psyc_comp + 
                          sns +
                          dd_mach_comp * sns + 
...
mach_model_fits <- map(mach_models, ~lmer(.x, data = data1)) 
...
narc_model_fits <- map(narc_models, ~lmer(.x, data = data1)) 
...
psyc_model_fits <- map(psyc_models, ~lmer(.x, data = data1)) 
